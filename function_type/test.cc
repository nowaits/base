...
public:
  ...
  static int WeakFun(int, bool, char){static int x ;return x;}
  static int& WeakFunA(int, bool, char){static int x ;return x;}
};
...
COMPILE_ASSERT(IS_RET_TYPE_OK(&TestClass::WeakFun, int(0)), okr3);
COMPILE_ASSERT(IS_RET_TYPE_OK(&TestClass::WeakFunA, int(0)), okr3);
